...
function navigationTabsInIt(listItems,listItemsActiveClass,contentItemShowClass){
    listItems.forEach(function(listItems){
        listItems.addEventListener("click",function(){
            ...
            let contentid=this.getAttribute("data_content_id")
...
navigationTabsInIt(resumewith,'resume_list_item_active','resume_content-show')
navigationTabsInIt(portfoliochoice,'portfolio_list_items_active','portfolio_content-show')
...
function observerHandler(allSections){
    ...
    allSections.map(section => {
      ...
      let sectonMenoItem=document.querySelector(`.meno_item[data_seactio=${sectionClassName}]`)
      console.log(section.isIntersecting)
      if(section.isIntersecting){
        sectonMenoItem.classList.add('meno_item_active')
      }
      else{
        sectonMenoItem.classList.remove('meno_item_active')
      }
...
changeThems.addEventListener('click',function(){
    document.documentElement.classList.toggle("dark_mode")
    if(document.documentElement.classList.contains('dark_mode')){
        window.localStorage.setItem("them","dark_mode")
        this.innerHTML=darkitemeicon;
    }else{
        window.localStorage.setItem("them","light_mode")
        this.innerHTML=lighttemeicon;
    }
})